...
function multiMap(arrVals, arrCallbacks) {
  ...
  arrVals.forEach(elm => {
    let values = [];
    values = arrCallbacks.map(fnElm => fnElm(elm));
    newObj[elm] = values;
  });
  return newObj;
}
...
function objOfMatchesWithArray(array1, array2, callback) {
  let modifiedArr = array1.map(elm => {
    return callback.reduce((acc, cv) => cv(acc), elm);
  });

  let matchedObj = {};
  for(let i = 0; i<array1.length; i++) {
    if(modifiedArr[i] == array2[i]){
      matchedObj[array1[i]] = array2[i];
    }
  }
  return matchedObj;
...
```js
function objOfMatchesWithArray(array1,  arrCallbacks) {
  let newObj = {};
  array1.forEach(elm => {
    let values = [];
    values = arrCallbacks.map(fnElm => fnElm(elm));
...
```js
function schedule(arrFn , arrSecs) {
  if(arrFn.length !== arrSecs.length) {
    alert('Invalid Input');
    return;
  }

  let timeInMilliSeconds = arrSecs.map(elm => elm * 1000);
